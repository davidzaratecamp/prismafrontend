...
export function MonthlyComparisonCard({ data }: { data?: RetellMonthlyComparison }) {
  ...
  const current = data.is_current_month
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-base">
          ...
          {current ? 'Mes en curso vs. anterior' : `${c.label} vs. mes anterior`}
        </CardTitle>
      </CardHeader>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <p className="text-xs text-muted-foreground">
              {c.label} {current ? `· ${c.days_elapsed}/${c.days_in_month} días` : '(completo)'}
            </p>
        ...
        <div className="rounded-lg border p-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-medium tabular-nums">
              {usd(current ? sp.cost_usd : p.cost_usd)}
            </span>
          </div>
          {data.same_period_change_pct != null ? (
            ...
            <p className="mt-1 text-[11px] text-muted-foreground">
              Sin base de comparación: {p.label} no registró actividad.
            </p>
          )}
        </div>
